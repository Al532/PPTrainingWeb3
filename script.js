...
function getCurrentExerciseType() {
  return (
    activeChromaSet?.exerciseType ||
    getExerciseTypeFromLabel(activeChromaSet?.label)
  );
}
...
function formatSeriesLabel(series) {
  ...
  const details = `${dateLabel} · ${count} trials`;
...
async function handleGenerateSeries() {
  ...
  if (!activeChromaSet?.chromas?.length) {
    updateSeriesStatus("Chroma set unavailable.", { autoHide: true });
    return;
  }
...
function refreshButtonOrder() {
  ...
  createButtons(activeChromaSet?.chromas ?? []);
...
async function init() {
  ...
  if (statsOutput) {
    statsOutput.textContent = "Stats will appear after a few answers.";
